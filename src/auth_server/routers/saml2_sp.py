...
async def authenticate(
    ...
    samlds: Optional[bool] = False,
    target: Optional[str] = None,
    entityid: Optional[str] = None,
):
    saml2_sp = await get_saml2_sp()
    if saml2_sp is None:
        # if there is no pysaml2 config or any database available no requests should get here
        raise HTTPException(status_code=400, detail="SAML authentication not configured")

    if samlds:
        # User has been at the discovery service
        if target is None or entityid is None:
            logger.error(f'Bad discovery service response. target: {target}, entity id: {entityid}')
            raise HTTPException(status_code=400, detail="bad discovery service response")
        if AuthnRequestRef(target) not in saml2_sp.authn_req_cache:
    ...
    if idp is None:
        ...
        return_url = f'{saml2_router.url_path_for("authenticate")}/?samlds=1&target={_authn_id}'
    ...
    _idp_redirect_url = await get_redirect_url(authn_request)
    logger.debug(f'Redirecting user to the IdP: {_idp_redirect_url}')
    return RedirectResponse(_idp_redirect_url, status_code=303)
